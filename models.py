...
class ChatSession(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    userUid = Column(String, nullable=True)  # Optional - for anonymous sessions
...
class ChatMessage(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    sessionId = Column(UUID(as_uuid=True), ForeignKey('chat_session.id', ondelete='CASCADE'), nullable=False)
    role = Column(String(20), nullable=False)  # 'user', 'assistant', 'system'
    content = Column(Text, nullable=False)
    createdAt = Column(DateTime(timezone=True), server_default=func.now())
    metadata = Column(JSON)  # Stores agent info, tokens, intent, etc.


class RecipeEmbedding(Base):
    """Vector embeddings for recipe semantic search"""
...
class IngredientEmbedding(Base):
    """Vector embeddings for ingredient semantic search"""
    __tablename__ = 'ingredient_embedding'

    ingredientId = Column(UUID(as_uuid=True), ForeignKey('ingredient.id', ondelete='CASCADE'), primary_key=True)
